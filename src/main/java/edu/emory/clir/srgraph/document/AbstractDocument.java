...
import edu.emory.clir.srgraph.core.Entity;
    ...
    public String toString()
    {
        ...
        for (Map.Entry<Integer,Instance> sentence: m_sentences.entrySet())
        {
            ...
            while(! q.isEmpty())
            {
                ...
                sb.append("Instance: " + getDEPNode(i).getWordForm() + ":\n");


                for (SemanticType type : i.getArgumentTypeSet()) {
                    for (Instance j : i.getArgumentList(type)) {
                        sb.append("has an argument relation " + type.toString() + " to -> " + getDEPNode(j).getWordForm() + "\n");
                ...
                for (SemanticType type : i.getPredicateTypeSet()) {
                    for (Instance j : i.getPredicateList(type)) {
                        sb.append("has a predicate relation " + type.toString() + " to -> " + getDEPNode(j).getWordForm() + "\n");
                    }
                }

                for (AttributeType type : i.getAttributeTypeSet()) {
                    for (Instance j : i.getAttributeList(type)) {
                        sb.append("has an attribute relation " + type.toString() + " to -> " + getDEPNode(j).getWordForm() + "\n");
                    }
                }
                sb.append("\n");
            }
        ...
        for (Map.Entry<Instance, Entity> entry: m_entities.entrySet())
        {
            sb.append("Instance: " + entry.getKey().getDepNode().getWordForm() + ", in entity: " + entry.getValue() + "\n");
        }
